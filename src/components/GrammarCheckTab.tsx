...
export function GrammarCheckTab() {
  const getSavedGrammarState = () => {
    try {
      const saved = localStorage.getItem('grammarTab_state');
  ...
  const savedState = getSavedGrammarState();
  ...
  const [tone, setTone] = useState<'Neutral' | 'Professional' | 'Empathetic' | 'Concise'>(savedState.tone || 'Neutral');
  const [isLoading, setIsLoading] = useState(false);
  const [result, setResult] = useState<GrammarCheckResult | null>(savedState.result || null);
  const [isCopied, setIsCopied] = useState(false);
  const [editedText, setEditedText] = useState(savedState.editedText || '');

  useEffect(() => {
    localStorage.setItem('grammarTab_state', JSON.stringify({ input, language, tone, result, editedText }));
  }, [input, language, tone, result, editedText]);
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
        <div className="flex items-center gap-3 mb-4">
          <div className="p-2 bg-primary-50 rounded-lg">
            <SpellCheck className="w-5 h-5 text-primary-600" />
          </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <select
                ...
                className="bg-slate-100 border-none text-sm font-medium text-slate-700 rounded-lg px-3 py-2 outline-none focus:ring-2 focus:ring-primary-500/20"
              >
