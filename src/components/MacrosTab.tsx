...
export function MacrosTab() {
  const { macros, deleteMacro, editMacro, saveMacro, toggleFavoriteMacro, translateLegacyMacros } = useAppContext();
    ...
    try {
      ...
      if (format === 'json') {
        ...
        const csv = Papa.unparse(macros.map(m => ({
          Summary: m.summary,
          Response: m.response,
          ResponseEn: m.responseEn || '',
          ResponseAr: m.responseAr || '',
        ...
        worksheet.columns = [
          { header: 'Summary', key: 'summary', width: 30 },
          { header: 'Response', key: 'response', width: 50 },
          { header: 'Response (En)', key: 'responseEn', width: 50 },
          { header: 'Response (Ar)', key: 'responseAr', width: 50 },
  ...
  const handleImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const processImport = async (items: { summary: string, response: string, responseEn?: string, responseAr?: string }[]) => {
      let count = 0;
      let skipped: string[] = [];
      ...
      for (const item of items) {
        if (item.summary && item.response) {
          ...
          if (existingSummaries.has(summaryLower)) {
            ...
            if (!item.responseEn || !item.responseAr) {
              await new Promise(resolve => setTimeout(resolve, 300));
            }
            saveMacro(item.summary, item.response, item.responseEn, item.responseAr);
        ...
        reader.onload = (evt) => {
          try {
            ...
            if (Array.isArray(parsed)) {
              processImport(parsed.map(m => ({ 
                summary: m.summary, 
                response: m.response,
                responseEn: m.responseEn || m.ResponseEn,
                responseAr: m.responseAr || m.ResponseAr
              })));
            }
          } catch (err) {
        ...
        Papa.parse(file, {
          ...
          complete: (results) => {
            const items = results.data.map((row: any) => ({
              ...
              responseEn: row.ResponseEn || row.responseEn || row['Response (En)'],
              responseAr: row.ResponseAr || row.responseAr || row['Response (Ar)']
            }));
            processImport(items);
          },
          error: () => {
        ...
        reader.onload = async (evt) => {
          try {
            ...
            firstRow.eachCell((cell, colNumber) => {
              ...
              if (val === 'response (en)' || val === 'responseen') responseEnCol = colNumber;
              if (val === 'response (ar)' || val === 'responsear') responseArCol = colNumber;
            });

            const items: { summary: string, response: string, responseEn?: string, responseAr?: string }[] = [];
            worksheet.eachRow((row, rowNumber) => {
              if (rowNumber === 1) return; // Skip header
              const summary = row.getCell(summaryCol).value?.toString();
              const response = row.getCell(responseCol).value?.toString();
              const responseEn = responseEnCol !== -1 ? row.getCell(responseEnCol).value?.toString() : undefined;
              const responseAr = responseArCol !== -1 ? row.getCell(responseArCol).value?.toString() : undefined;
              if (summary && response) {
                items.push({ summary, response, responseEn, responseAr });
              }
            });
            processImport(items);
          } catch (err) {
      ...
      <div className="sticky top-0 z-10 bg-slate-50/80 backdrop-blur-md pb-4 pt-2 -mt-2">
        <div className="bg-white/90 backdrop-blur-sm rounded-2xl shadow-lg shadow-slate-200/40 border border-slate-200/60 p-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
            ...
            <Button
              variant="secondary"
              onClick={translateLegacyMacros}
              className="bg-white shadow-sm font-medium"
            >
              Translate Legacy Macros
            </Button>
            ...
            <div className="relative" ref={exportMenuRef}>
              ...
              {showExportMenu && (
                <div className="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-lg border border-slate-200 z-20 py-1">
                  ...
                  <button onClick={() => exportMacros('template')} className="block w-full text-left px-4 py-2 text-sm text-primary-600 font-medium hover:bg-primary-50">Download Template</button>
                </div>
              )}
            </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <select
            ...
            className="px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 text-slate-700 font-medium outline-none cursor-pointer"
          >
        ...
        <AnimatePresence mode="popLayout">
          {isCreating && (
            <motion.div 
              ...
              className="bg-white border border-primary-200 rounded-xl overflow-hidden shadow-sm ring-1 ring-primary-500/20"
            >
                  ...
                  <div className="relative">
                    {editingId === macro.id ? (
                      ...
                      <div className="space-y-4">
                        {(macro.responseEn || macro.responseAr) ? (
                          <>
                            {macro.responseEn && (
                              <div className="relative">
                                <h4 className="text-xs font-semibold text-slate-500 mb-1">English</h4>
                                <pre 
                                  ...
                                  dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(macro.responseEn) }}
                                />
                                <Button
                                  variant="secondary"
                                  size="sm"
                                  onClick={() => handleCopy(macro.id + '-en', macro.responseEn!)}
                                  className="absolute top-6 right-2"
                                  leftIcon={copiedId === macro.id + '-en' ? <Check className="w-4 h-4 text-emerald-600" /> : <Copy className="w-3.5 h-3.5" />}
                                >
                                  {copiedId === macro.id + '-en' ? <span className="text-emerald-600">Copied!</span> : 'Copy'}
                                </Button>
                              </div>
                            )}
                            {macro.responseAr && (
                              <div className="relative" dir="rtl">
                                <h4 className="text-xs font-semibold text-slate-500 mb-1" dir="ltr">Arabic</h4>
                                <pre 
                                  className="p-4 bg-white rounded-lg border border-slate-200 text-slate-700 whitespace-pre-wrap font-sans text-sm leading-relaxed text-right"
                                  dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(macro.responseAr) }}
                                />
                                <Button
                                  variant="secondary"
                                  size="sm"
                                  onClick={() => handleCopy(macro.id + '-ar', macro.responseAr!)}
                                  className="absolute top-6 left-2"
                                  leftIcon={copiedId === macro.id + '-ar' ? <Check className="w-4 h-4 text-emerald-600" /> : <Copy className="w-3.5 h-3.5" />}
                                >
                                  {copiedId === macro.id + '-ar' ? <span className="text-emerald-600">Copied!</span> : 'Copy'}
                                </Button>
                              </div>
                            )}
                          </>
                          ...
                          <div className="relative">
                            ...
                            <Button
                              ...
                              leftIcon={copiedId === macro.id ? <Check className="w-4 h-4 text-emerald-600" /> : <Copy className="w-3.5 h-3.5" />}
                            >
                              {copiedId === macro.id ? <span className="text-emerald-600">Copied!</span> : 'Copy'}
                            </Button>
                          </div>
                        )}
                      </div>
