...
export function RephraseTab() {
  const getSavedRephraseState = () => {
    try {
      const saved = localStorage.getItem('rephraseTab_state');
  ...
  const savedState = getSavedRephraseState();

  const [input, setInput] = useState(savedState.input || '');
  const [tone, setTone] = useState(savedState.tone || 'Professional');
  const [result, setResult] = useState(savedState.result || '');
  const [isLoading, setIsLoading] = useState(false);
  const [isCopied, setIsCopied] = useState(false);

  useEffect(() => {
    localStorage.setItem('rephraseTab_state', JSON.stringify({ input, tone, result }));
  }, [input, tone, result]);
