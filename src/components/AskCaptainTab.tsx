...
export function AskCaptainTab() {
  const getSavedAskCaptainState = () => {
    try {
      const saved = localStorage.getItem('askCaptainTab_state');
  ...
  const savedState = getSavedAskCaptainState();

  const [input, setInput] = useState(savedState.input || '');
  const [isLoading, setIsLoading] = useState(false);
  const [result, setResult] = useState<CaptainRequestResult | null>(savedState.result || null);
  const [isCopied, setIsCopied] = useState(false);

  // Editable fields state
  const [formValues, setFormValues] = useState(savedState.formValues || {
    ticketLink: '',
    summary: '',
    validation: '',
    needsFromCaptain: ''
  ...
  useEffect(() => {
    localStorage.setItem('askCaptainTab_state', JSON.stringify({ input, result, formValues }));
